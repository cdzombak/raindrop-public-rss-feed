...
BIN_NAME:=raindrop-public-rss-feed
...
build-linux-armv7: ## Build for Linux/armv7 to ./out
	mkdir -p out
	env CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=7 go build -ldflags="-X main.version=${BIN_VERSION}" -o ./out/${BIN_NAME}-${BIN_VERSION}-linux-armv7 ${BUILD_PKG}

.PHONY: build-linux-armv6
build-linux-armv6: ## Build for Linux/armv6 to ./out
	mkdir -p out
	env CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=6 go build -ldflags="-X main.version=${BIN_VERSION}" -o ./out/${BIN_NAME}-${BIN_VERSION}-linux-armv6 ${BUILD_PKG}
